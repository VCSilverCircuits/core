...
public class BindingManager {
  ...
  public void setBindings(
      RobotMode mode, BindingSet gamepad1Bindings, BindingSet gamepad2Bindings) {
    this.gamepad1Bindings.put(mode, gamepad1Bindings);
    this.gamepad2Bindings.put(mode, gamepad2Bindings);
  }
      ...
      if (gamepad1.debounceIsPressed(btn)) {
        ...
        if (task != null) {
          System.out.println(
              "[BindingManager::loop] "
                  + btn
                  + " pressed on Gamepad1. Triggering task "
                  + task.getClass().getSimpleName());
          TaskManager.getInstance().runTask(task);
        } else {
          System.out.println(
              "[BindingManager::loop] " + btn + " pressed on Gamepad1. No task bound.");
        }
      }
      if (gamepad2.debounceIsPressed(btn)) {
        ...
        if (task != null) {
          System.out.println(
              "[BindingManager::loop] "
                  + btn
                  + " pressed on Gamepad2. Triggering task "
                  + task.getClass().getSimpleName());
          TaskManager.getInstance().runTask(task);
        } else {
          System.out.println(
              "[BindingManager::loop] " + btn + " pressed on Gamepad2. No task bound.");
        }
      }
    }
  }
