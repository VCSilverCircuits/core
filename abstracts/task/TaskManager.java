...
public class TaskManager {
  ...
  public boolean runTask(Task task, boolean noCancel) {
    if (!noCancel) {
      for (Task other : runningTasks) {
        if (task.conflictsWith(other)) {
          System.out.println(
              "[TaskManager::runTask] WARNING: Task "
    ...
    if (started) {
      ...
      System.out.println(
          "[TaskManager::runTask] Successfully started task: " + task.getClass().getSimpleName());
    } else {
      System.out.println(
          "[TaskManager::runTask] Failed to start task: " + task.getClass().getSimpleName());
    }
    return started;
  }
  ...
  public void clearTasks() {
    ...
    for (Task task : runningTasks) {
      System.out.println(
          "[TaskManager::clearTasks] Canceling task: " + task.getClass().getSimpleName());
      task.cancel();
    }
    runningTasks.clear();
  }
  ...
  public void cancelTask(Task task) {
    if (runningTasks.contains(task)) {
      System.out.println(
          "[TaskManager::cancelTask] Canceling task: " + task.getClass().getSimpleName());
      task.cancel();
      runningTasks.remove(task);
    } else {
      System.out.println(
          "[TaskManager::cancelTask] Trying to cancel task: "
              + task.getClass().getSimpleName()
              + ", but not task is not running");
    }
  }
  ...
  public void loop() {
    ...
    System.out.println(
        "[TaskManager::loop] Currently running tasks (" + runningTasks.size() + "):");
    ...
    for (Task task : runningTasks) {
      ...
      if (task.isFinished()) {
        System.out.println(
            "[TaskManager::loop] Task finished: "
                + task.getClass().getSimpleName()
